...
static void get_full_comm(char *comm_buf, size_t buf_len)
{
	...
	if (!ksu_access_ok((void __user *)arg_start, len))
		goto fallback;
	...
	const char *name = slash && *(slash + 1) ? slash + 1 : cmdline;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 13, 0)
		strscpy(comm_buf, name, buf_len);
#else
		strlcpy(comm_buf, name, buf_len);
#endif
...
static void safe_copy_comm(char *full_comm, const char *comm, size_t buf_len)
{
    if (comm && strlen(comm) > 0) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 13, 0)
        strscpy(full_comm, comm, buf_len - 1);
#else
        strlcpy(full_comm, comm, buf_len - 1);
#endif
        full_comm[buf_len - 1] = '\0';
    } else {
        get_full_comm(full_comm, buf_len);
    }
}
...
void ksu_sulog_report_su_grant(uid_t uid, const char *comm, const char *method)
{
	...
	safe_copy_comm(full_comm, comm, SULOG_COMM_LEN);
